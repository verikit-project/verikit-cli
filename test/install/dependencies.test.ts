...
test("runCommand resolves ok:true and captures stdout on success", async () => {
  ...
  try {
    const result = await runCommand(process.execPath, ["-e", "console.log('hello')"], dir);
...
test("runCommand resolves ok:false on a non-zero exit code", async () => {
  const dir = makeFixture();
  try {
    const result = await runCommand(process.execPath, ["-e", "process.exit(1)"], dir);
...
test("runCommand resolves ok:false when the command can't be spawned", async () => {
  const dir = makeFixture();
  try {
    const result = await runCommand("verikit-cli-definitely-not-a-real-binary", [], dir);
    assert.equal(result.ok, false);
...
test("installPackages delegates to installCommand and the injected runner", async () => {
  ...
  const result = await installPackages("pnpm", ["@verikit/server@^0.23.1"], "/some/dir", fakeRun);

  assert.equal(result.ok, true);
  assert.deepEqual(calls, [
    { command: "pnpm", args: ["add", "@verikit/server@^0.23.1"], cwd: "/some/dir" },
  ]);
});
